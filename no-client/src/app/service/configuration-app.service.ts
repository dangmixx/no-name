...
export class ConfigurationAppService {
    ...
    public loadConfig() {
        ...
        const headers = new HttpHeaders().set('client-cache-expire-time', timeExpire.toISOString()).set('client-cache-type', 'local').set('client-cache', 'true').set('api-url-config', 'local');

        this.http.get(configFileUrl, { headers: headers }).pipe(
            retryWhen(errors => errors.pipe(delay(1000), tap(error => {
